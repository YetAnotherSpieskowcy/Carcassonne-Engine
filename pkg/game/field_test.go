...
func TestScoreFieldSimple(t *testing.T) {
	/*
		...
		The meeple is placed on the field feature in the monastery tile (position: 1,0).
	*/
	...
	if field.CitiesCount() != 0 {
		t.Fatalf("expected %#v, got %#v instead", 0, field.CitiesCount())
	}
...
func TestScoreFieldTwoPlayersGetPoints(t *testing.T) {
	/*
		...
		(two cities are not closed - the city on the left of the bottom monastery and the right part of the city that has two city features on one tile)
	...
	*/
	...
	if field.CitiesCount() != 2 {
		t.Fatalf("expected %#v, got %#v instead", 2, field.CitiesCount())
	}
